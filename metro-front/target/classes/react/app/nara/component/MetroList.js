...
class MetroList extends Component {
  ...
  renderViewMetro(index, metro) {
    //
    const { editingExistent, onEditCitizen, onViewCitizen, onClickMetro } = this.props;
    const adminExistent = metro.admins && metro.admins.primaryAdmin;

    return (
      <tr className="text-center" key={index} onClick={() => onClickMetro(metro)} style={{ cursor: 'pointer' }}>
        <td>{index + 1}</td>
        <td>{metro.name}</td>
        <td>{metro.memo}</td>
